import { detectSegments, type DetectedLine } from './wallDetect';
 ...
 */
export type AutoTraceResult = { lines: DetectedLine[]; wallCount: number };
...
export function autoTraceImage(
  ...
  return new Promise((resolve) => {
    const empty: AutoTraceResult = { lines: [], wallCount: 0 };
    const img = new Image();
    img.onload = () => {
      try {
        ...
        const lines = detectSegments(grid, gw, gh, {
          ...
          .filter((l) => (l.points[0].y + l.points[1].y) / 2 < captionTop);
        resolve({ lines, wallCount: lines.length });
      } catch {
        resolve(empty);
      }
    };
