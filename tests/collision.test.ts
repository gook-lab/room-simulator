...
import { PLAYER_RADIUS, buildColliders, resolveCollisions } from '../src/features/three/collision';
...
describe('resolveCollisions (캡슐 0.25m)', () => {
  ...
  it('가구 모서리 콜라이더에 막힘', () => {
    const plan = createSamplePlan();
    const cs = buildColliders(plan);
    // 주방 라운드 테이블(7.6, 4.35, Ø1.1) 좌측 변에 반경보다 가깝게 접근
    ...
    expect(leftEdge - res.x).toBeGreaterThanOrEqual(PLAYER_RADIUS - 1e-6);
  });
});
