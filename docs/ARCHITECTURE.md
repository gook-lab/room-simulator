...
## 다층 — 층 = buildingId로 연결된 독립 문서

층은 `Plan` 안의 배열이 아니라, 같은 `buildingId`를 가진 **독립 문서들의 묶음**
입니다(`Plan.buildingId` + `floorLabel`, 정렬·y 오프셋은 `model/floorStack.ts`).

Plan 내부에 `floors[]`를 두는 방식을 검토 후 기각했습니다. 지오메트리·충돌·견적·
2D/3D 렌더 등 SSOT에서 파생되는 모든 계산이 "현재 층 필터"를 한 겹씩 통과해야
해서 파생 경로 전체가 수술 대상(=회귀 위험)이 되고, 저장 포맷도 v2 마이그레이션이
필요해집니다. 연결 문서 방식은 기존 엔진이 층 단위로 그대로 동작하고, `buildingId`
·`floorLabel`이 optional이라 **planIO v1 파일이 무변경 호환**됩니다. 층 전환은
문서 전환(`switchFloor`, view 유지)이고, 조감도 층 스택은 렌더 계층에서 층 문서들을
y 오프셋(아래층 층고 합)으로 겹쳐 그리는 것뿐이라 SSOT는 손대지 않습니다.

## 저장 — localStorage 방어 3겹

저장소는 전체 스냅샷 1키(last-write-wins)라, 비정상 상태가 한 번 덮으면 문서가
복구 불가로 사라집니다. 실제로 로드 실패 → 샘플 폴백 → 다음 저장이 기존 저장본을
샘플로 덮어쓰는 소실 사건이 있었고(원인 규명 후), `state/persistence.ts`에 세 겹의
방어를 두었습니다.

1. **병합 가드** — 저장 직전 기존 저장본과 비교해, 의도된 삭제(삭제 액션이 마킹)가
   아닌데 사라지는 문서는 되살려 저장하고 메모리에도 반영. 같은 id는 메모리 우선
   (undo로 되돌린 내용도 저장돼야 하므로 updatedAt 비교는 하지 않음).
2. **백업 1세대** — 덮어쓰기 전 이전 저장본을 `.bak` 키로 회전. 메인 손상 시 로드가
   백업으로 폴백하고, 손상 원본은 `.corrupt` 키로 보존.
3. **용량 실패 표면화** — quota 초과는 백업을 비우고 1회 재시도, 그래도 실패하면
   상단바 배지로 알리고 JSON 백업을 안내(조용한 무시 제거).
